...
class Question(db.Model):
    ...
    answer = db.Column(db.String(100), nullable=False)  # Добавляем это поле
...
class Result(db.Model):
    ...
    # Связи, чтобы легко доставать имена
...
def add_question():
    ...
    if request.method == 'POST':
        ...
        answer = request.form['answer']  # Забираем ответ
        quiz_id = request.form['quiz_id']

        # Добавляем answer в конструктор
        new_question = Question(text=text, answer=answer, quiz_id=quiz_id)
        db.session.add(new_question)
        db.session.commit()
        return redirect(url_for('questions'))
    return render_template('questions/add.html', quizzes=quizzes)
...
def play_quiz(quiz_id):
    ...
    # Находим все вопросы, которые вы добавили именно для этой викторины
    questions = Question.query.filter_by(quiz_id=quiz_id).all()
    return render_template('quizzes/play.html', quiz=quiz, questions=questions)
